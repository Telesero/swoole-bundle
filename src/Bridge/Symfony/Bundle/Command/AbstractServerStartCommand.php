...
//        $this->serverConfiguration->lock();
    ...
    protected function startServer(HttpServerConfiguration $serverConfiguration, HttpServer $server, SymfonyStyle $io): void
    {
        ...
        if ($this->pcntlSignalHandler instanceof PcntlSignalHandler && $serverConfiguration->isReactorRunningMode()) {
            ...
            $this->pcntlSignalHandler->register(function () use ($srv): void {
                dump(\sprintf('pcntl hit 2; master: %d, manager: %d, mypid: %d', $srv->master_pid, $srv->manager_pid, \getmypid()));
                if (\getmypid() !== $srv->master_pid) {
                    $srv->shutdown();
                }
            }, Signal::int());
        }

        $server->getServer()->on('WorkerExit', function (): void {
